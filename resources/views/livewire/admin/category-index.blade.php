<div wire:key="category-index-root" class="animate-in fade-in duration-300 relative w-full pb-10">
    <!-- Breadcrumbs Section -->
    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 tracking-tight">Kategori</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola grup kategori produkmu di sini</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <!-- Name Column -->
                        <th scope="col"
                            class="px-3 py-2 text-xs font-medium text-gray-500 border-r border-gray-200 sm:w-auto min-w-[200px]">
                            <div class="flex items-center space-x-1.5">
                                <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @forelse ($categories as $category)
                                    <tr wire:key="category-row-{{ $category->id }}"
                                        ...
                                        <td class="px-3 py-2.5 border-r border-gray-200">
                                            <span
                                                class="text-[13px] font-semibold text-gray-900 block truncate">{{ $category->name }}</span>
                                        </td>
                                        ...
                                        <td class="hidden md:table-cell px-3 py-2.5 border-r border-gray-200">
                                            <span class="text-[13px] text-gray-500 block truncate max-w-xs"
                                                title="{{ $category->description }}">{{ $category->description ?? '-' }}</span>
                                        </td>

                                        <!-- Status Label (Clickable) -->
                                        <td class="w-px whitespace-nowrap px-3 py-2.5 border-r border-gray-200 text-center">
                                            <button wire:click="toggleActive({{ $category->id }})" class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase transition-all
                                                                    {{ $category->is_active
                        ...
                        : 'bg-slate-100 text-slate-500 border border-slate-200/60 hover:bg-slate-200 shadow-sm' }}">
                                        ...
                                        <td class="px-3 py-2.5 text-center w-24 whitespace-nowrap">
                                            <div class="flex items-center justify-center gap-1.5">
                                                <a href="{{ route('admin.categories.edit', $category->id) }}" wire:navigate title="Edit"
                                                ...
                                                <button type="button"
                                                    @click="$dispatch('open-delete-modal', { name: '{{ addslashes($category->name) }}', url: '{{ route('admin.categories.destroy', $category->id) }}' })"
                                                    title="Delete"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-10 text-center">
                                ...
                                <p class="mt-1 text-xs text-gray-500">Silakan tambahkan kategori baru.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <div class="bg-white px-5 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-lg font-bold text-gray-900 tracking-tight">Hapus Kategori</h3>
                            <div class="mt-2">
                                <p class="text-[14px] text-gray-500 font-medium">
                                    Apakah Anda yakin ingin menghapus kategori <strong x-text="itemName"
